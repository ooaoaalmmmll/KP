...
func (p *Item) GetItemsByOwner() (items []Item, err error) {
	items = make([]Item, 0)
	rows, err := db.Query("SELECT id, address, date, dims, desc, ownerId FROM WHERE ownerId=?", p.OwnerId)
	if err != nil {
		return
	}
...
func GetItemsByOwnerApi(c *gin.Context) {
	var p Item
	items, err := p.GetItemsByOwner()
	if err != nil {
		log.Fatalln(err)
	}
